...
def draw(draw_info,algo_name,ascending):
    ...
    title = draw_info.LARGE_FONT.render(f"{algo_name} Algorithm - {'Ascending' if ascending else 'Descending'}",1,draw_info.YELLOW)
...
def insertion_sort(draw_info,ascending= True):
    ...
    for i in range(1,len(lst)):
        ...
        while True:
            ...
            i = i-1
            lst[i] = current
            color_positions = {i:draw_info.GREEN,i-1:draw_info.RED}
            draw_list(draw_info,color_positions,True)
            yield
    return lst
...
def main():
    ...
    draw_info = DrawInformation(900,500,lst)
    sorting = False
    ascending = True
